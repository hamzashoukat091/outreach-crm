...
export function SignOutButton({ compact = false }: { compact?: boolean } = {}) {
  ...
  // Icon only on mobile -- the label is redundant next to a door-and-arrow
  // glyph, and the brand row has no width to spare.
  if (compact) {
    return (
      <button
        onClick={signOut}
        ...
        aria-label="Sign out"
        title="Sign out"
        className="btn-ghost min-h-11 w-11 shrink-0 px-0"
      >
        {icon}
      </button>
    );
